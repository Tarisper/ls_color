...
function Write-Color-LS {
  param ([string]$color = "white", $file, $pref, $IsFile)
  if ($IsFile) { $size = DisplayInBytes $file.length }
  else { $size = '' }
  Write-host ("{0,-7} {1,25} {2,10} {3}" -f $file.mode, ([String]::Format("{0,10}  {1,8}", $file.LastWriteTime.ToString("d"), $file.LastWriteTime.ToString("t"))), $size, $pref + $file.name) -foregroundcolor $color 
}
...
New-CommandWrapper Out-Default -Process {
  ...
  $executable = New-Object System.Text.RegularExpressions.Regex(
    '\.(exe|bat|cmd|py|pl|ps1|psm1|vbs|rb|reg|fsx|sh|o)$', $regex_opts)
  $dll_pdb = New-Object System.Text.RegularExpressions.Regex(
    '\.(dll|pdb|so)$', $regex_opts)
  $configs = New-Object System.Text.RegularExpressions.Regex(
    '\.(config|conf|ini|json)$', $regex_opts)
  $text_files = New-Object System.Text.RegularExpressions.Regex(
    '\.(txt|cfg|conf|ini|csv|log)$', $regex_opts)
  $images = New-Object System.Text.RegularExpressions.Regex(
    '\.(jpg|jpeg|gif|bmp|png)$', $regex_opts)
  $codes = New-Object System.Text.RegularExpressions.Regex(
    '\.(cpp|hpp|c|h)$', $regex_opts)
  ...
  if (($_ -is [System.IO.DirectoryInfo]) -or ($_ -is [System.IO.FileInfo])) {
    ...
    if ($_ -is [System.IO.DirectoryInfo]) {
      Write-Color-LS "White" $_ ' ' $FALSE               
    }
    elseif ($compressed.IsMatch($_.Name)) {
      Write-Color-LS "DarkRed" $_ ' ' $TRUE
    }
    elseif ($executable.IsMatch($_.Name)) {
      Write-Color-LS "Green" $_ ' ' $TRUE
    }
    elseif ($dll_pdb.IsMatch($_.Name)) {
      Write-Color-LS "DarkGreen" $_ ' ' $TRUE
    }
    elseif ($text_files.IsMatch($_.Name)) {
      Write-Color-LS "DarkCyan" $_ ' ' $TRUE
    }
    elseif ($configs.IsMatch($_.Name)) {
      Write-Color-LS "Blue" $_ ' ' $TRUE
    }
    elseif ($images.IsMatch($_.Name)) {
      Write-Color-LS "Red" $_ ' ' $TRUE
    }
    elseif ($codes.IsMatch($_.Name)) {
      Write-Color-LS "Yellow" $_ ' ' $TRUE
    }
    else {
      Write-Color-LS "DarkGray" $_ ' ' $TRUE
    }
